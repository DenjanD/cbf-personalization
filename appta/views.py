...
def fetch_videos(query, max_videos):
    ...
    while len(video_details) < max_videos:
        request = youtube.search().list(
            part="snippet",
            maxResults=50,  # Ambil hasil pencarian per halaman
            order="date",
            type="video",
            q=query,
        ...
        if video_ids:
            ...
            for item in video_response['items']:
                ...
                # Parse durasi dari format ISO 8601
                duration_seconds = parse_iso_duration(duration)
                if duration_seconds >= 60:
                    video_item = next(v for v in response['items'] if v['id']['videoId'] == video_id)
        ...
        if next_page_token is None or len(video_details) >= max_videos:
            break
...
def load_and_prepare_data(query, videoQty):
    ...
    video_data_cleaned = video_data.drop(columns=['id', 'snippet'])
